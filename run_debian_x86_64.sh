...
prepare_rootfs(){
	...
	# hostname = linux3-x64
	echo "linux3-x64" > "${ROOTFS_PATH}/etc/hostname"
...
build_rootfs(){
	...
	mkfs.ext3 -F rootfs_debian_x86_64.ext3
	mkdir -p tmpmount
	echo "copy data into rootfs..."
	mount -t ext3 rootfs_debian_x86_64.ext3 tmpmount/ -o loop
...
case $1 in
	...
	onekey)
		...
		if [ -f rootfs_debian_x86_64.ext3 ]
		then
