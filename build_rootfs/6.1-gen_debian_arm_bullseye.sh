...
then
	...
	# then
	# 	tar -xvf qemu-arm-static.tar.gz -C /bin/
	# 	chmod +x /bin/qemu-arm-static
	# fi
fi
...
then
	...
	debootstrap --arch="armel"  ${FROEIGN}  \
		--no-check-gpg \
...
if [ "${CUR_ARCH}" != "${ARCH}" ]
then
	cp -a /bin/qemu-arm-static "${OUTPUT}/usr/bin/"
	cd ${OUTPUT}
...
# 修改主机名
echo "linux6-arm" > "${OUTPUT}/etc/hostname"
echo " * set hostname : linux6-arm"
# 修改bash环境
